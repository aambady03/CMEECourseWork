...
P <- ggplot(ecol_archives,
  ...
  labs(
    x = expression(paste(log[10], "(Prey mass in grams)")),
    y = expression(paste(log[10], "(Predator mass in grams)")),
    color = "Predator Lifestage",
    title = paste0("Log-Log Predator-Prey Mass Relationship")
  ) +
...
ggsave(results_pdf_path, P, width = 11, height = 8) 
